...
function useCanvasMode() {
  const [mode, setMode] = useState<{ skip: boolean; fullBleed: boolean } | null>(
    null,
  );
...
export function FestiLanyardSlot() {
  ...
  if (!mode) {
    return <div className="home-launch__lanyard" aria-hidden />;
  }

  if (mode.skip) return <StaticPass />;

  return (
    <LanyardErrorBoundary fallback={<StaticPass />}>
      <div
        className="home-launch__lanyard"
        data-full-bleed={mode.fullBleed ? "" : undefined}
      >
        <Suspense fallback={null}>
          <FestiLanyard key={mode.fullBleed ? "full" : "inline"} fullBleed={mode.fullBleed} />
        </Suspense>
      </div>
